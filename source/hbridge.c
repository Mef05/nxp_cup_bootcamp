...
void HbridgeSpeed(Hbridge *h, int16_t speed1, int16_t speed2)
{
    ...
    // Dacă mergem înainte (speed >= 0), dir1=0 (Coast). Power = HIGH time -> duty = p
    // Dacă mergem înapoi (speed < 0), dir1=1 (Brake). Power = LOW time -> duty = 100 - p
    uint8_t duty1 = (dir1 == 0U) ? p1 : (100U - p1);

    uint8_t dir2 = (speed2 >= 0) ? 0U : 1U;
    uint8_t p2   = (uint8_t)((speed2 >= 0) ? speed2 : -speed2);
    uint8_t duty2 = (dir2 == 0U) ? p2 : (100U - p2);
